...
begin
	...
	select price into n from bus@site1 where bid_f = bus.bid;
...
create or replace procedure create_Transaction(p_tid in transactions.tid%type,b_bid in bus.bid%type,p_name in transactions.pname%type,p_number in transactions.phn%type,
	...
	begin
		...
		insert into transactions@site1(tid, bid, rid, sid, pname, phn, quantity, amount) values (p_tid, b_bid, p_rid, p_sid , p_name , p_number, p_quantity, p_amount);
		exception 
...
begin
	...
	select avail into n from bus@site1 where bid_f = bus.bid;

	if n-num >= 0 then
		n := n-num;
		update bus@site1 set avail = n where bid_f = bus.bid;
		commit;
	elsif n-num < 0 then
		raise error;
	end if;
...
begin
	...
	FOR R IN (select transactions.tid FROM transactions@site1) 
		LOOP
			Tidd := R.tid;
		END LOOP;
	Tidd := Tidd + 1;	
