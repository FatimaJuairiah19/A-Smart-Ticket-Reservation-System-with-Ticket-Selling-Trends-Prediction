...
begin
	...
	select price into n from bus where bus.bname = busname;
...
begin
	...
	if tseats = n then
		...
		update transactions set quantity = quantity - tseats WHERE tid = Tidd;
	end if;	
	commit;
...
begin 
	...
	select tid into Tidd FROM transactions where pname = name ;	
